...
def check_version(
    ...
    if not current:  # if current is '' or None
        ...
        return True
    elif not current[0].isdigit():  # current is package name rather than version string'
        try:
...
def check_suffix(file="aran.pt", suffix=".pt", msg=""):
...
def check_aran(verbose=True, device=""):
    """Return a human-readable YOLO software and hardware summary."""
    import psutil

    from aran.utils.torch_utils import select_device
...
def collect_system_info():
    ...
    from aran.utils import ENVIRONMENT, is_git_dir
    from aran.utils.torch_utils import get_cpu_info
    ...
    for r in parse_requirements(package="aran"):
        try:
...
def check_amp(model):
    ...
    try:
        ...
        assert amp_allclose(ARAN("aran.pt"), im)
    ...
    # except ConnectionError:
        ...
        LOGGER.warning(
            ...
            # f"Unable to load YOLOv8n due to possible aran package modifications. {warning_msg}"
        )
...
def print_args(args: Optional[dict] = None, show_file=True, show_func=False):
    ...
    def strip_auth(v):
        """Clean longer aran HUB URLs by stripping potential authentication information."""
